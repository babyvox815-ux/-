(() => {
  ...
  const 저장키='용호운동-v23';
  const 부위데이터=[
    {id:'하체',위치:'67% 5%',운동:[['스쿼트',4,'6–10회',150,'3% 61%'],['레그프레스',4,'10–15회',120,'3% 68%'],['레그 익스텐션',3,'12–15회',75,'3% 74%'],['레그 컬',3,'10–15회',90,'3% 81%']]},
    {id:'어깨',위치:'98% 5%',운동:[['숄더 프레스',4,'8–12회',120,'3% 88%'],['사이드 레터럴 레이즈',3,'12–20회',60,'3% 95%'],['리버스 펙덱',3,'12–20회',60,'98% 5%']]},
    {id:'가슴',위치:'67% 18%',운동:[['벤치프레스',4,'6–10회',150,'67% 18%'],['인클라인 프레스',3,'8–12회',120,'67% 18%'],['케이블 플라이',3,'12–15회',75,'67% 18%']]},
    {id:'등',위치:'98% 18%',운동:[['랫풀다운',4,'8–12회',120,'98% 18%'],['시티드 케이블 로우',4,'8–12회',120,'98% 18%'],['원암 덤벨 로우',3,'10–12회',90,'98% 18%']]},
    {id:'이두',위치:'67% 29%',운동:[['바벨컬',3,'8–12회',90,'67% 29%'],['해머컬',3,'10–15회',75,'67% 29%']]},
    {id:'삼두',위치:'98% 29%',운동:[['케이블 푸시다운',4,'10–15회',75,'98% 29%'],['오버헤드 익스텐션',3,'10–15회',75,'98% 29%']]},
    {id:'복근',위치:'67% 39%',운동:[['케이블 크런치',3,'12–15회',60,'67% 39%'],['행잉 니레이즈',3,'10–15회',60,'67% 39%'],['플랭크',3,'30–60초',60,'67% 39%']]},
    {id:'유산소',위치:'98% 39%',운동:[['트레드밀 걷기',1,'20–30분',0,'98% 39%'],['인터벌 러닝',1,'12–20분',0,'98% 39%']]}
  ];
  ...
  const 원본=window.YGO_AI_HERO||window.YGO_IMAGES?.hero||'';
  function 배경(크기,위치){return `background-image:url('${원본}');background-size:${크기};background-position:${위치};background-repeat:no-repeat`}
  function 알림(t){const e=$('#알림');e.textContent=t;e.classList.add('보임');setTimeout(()=>e.classList.remove('보임'),1500)}
  function 선택운동(){const d=날짜기록();return d.부위.flatMap(id=>{const p=부위데이터.find(x=>x.id===id);return p?p.운동.map((x,i)=>({부위:id,이름:x[0],세트:x[1],반복:x[2],휴식:x[3],위치:x[4],키:`${id}-${i}`})):[]})}
  function 그리기(){
    ...
    $('#날짜입력').value=날짜;
    $('#대표사진').setAttribute('style',배경('200% auto','0% 23%'));
    $('#부위목록').innerHTML=부위데이터.map(p=>`<button class="부위버튼 ${d.부위.includes(p.id)?'선택됨':''}" data-part="${p.id}"><span class="부위체크"></span><span class="부위사진" style="${배경('445% auto',p.위치)}"></span><span class="부위그늘"></span><span class="부위내용"><h3>${p.id}</h3><p>${p.운동.length}개 운동</p></span></button>`).join('');
