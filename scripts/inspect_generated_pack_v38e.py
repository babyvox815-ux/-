...
for path in sorted(CHUNK_DIR.glob("c*.js")):
    ...
    metadata.append(f"{path.name}: chars={len(encoded)} padding={encoded.count('=')}")

if not chunks:
    raise RuntimeError("No generated image chunks found")

# The historic upload split the source base64 at arbitrary boundaries and left
# padding markers inside the sequence. Remove those internal markers and pad once.
